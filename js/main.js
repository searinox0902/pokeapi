...
const observerRequest = document.getElementById('item_wrap');
let pokeList = [];
let pokemon = {};
...
observer.observe(observerRequest);
...
async function makeRequest() {
  ...
  await fetch(apiUrl)
        ...
        .then(data => {
          ...
          requestPoke(data.results);
          renderPokemons();
        });
  
}

function requestPoke(e) {
  e.forEach(element => {
    fetch(element.url)
      .then(res => res.json())
      .then(data => {
        const algo = Object.entries(data)
        pokeList.push(data)
      });
  });
}
...
(async () => {
  ...
  try {
    for (let i = 1; i < NUMBERPOKEMONS; i++) {
      const pokeApi =  `https://pokeapi.co/api/v2/pokemons/${apiUrl}`;
